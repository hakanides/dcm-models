#!/usr/bin/env python3
"""
MNL Demographics Model Estimation
=================================

Estimates MNL with demographic interactions on fee and duration sensitivity.
...
def create_model(database: db.Database):
    """Create MNL Demographics model specification."""
...
def estimate(model_dir: Path, verbose: bool = True) -> dict:
    """Estimate MNL Demographics model and compare to true parameters."""
    # Paths
    ...
    if verbose:
        ...
        print("=" * 70)
        print(f"\n{'Parameter':<12} {'True':>10} {'Estimated':>10} {'Bias%':>10} {'95% CI':<10}")
        print("-" * 60)
        ...
        for param in param_order:
            if param in betas and param in true_values:
                ...
                print(f"{param:<12} {true_val:>10.4f} {est_val:>10.4f} {bias_pct:>+9.1f}% {coverage_str:<10}")
    ...
    for param in betas.keys():
        ...
        bias_pct = ((betas[param] - true_val) / abs(true_val)) * 100 if true_val and abs(true_val) > 0.001 else None
