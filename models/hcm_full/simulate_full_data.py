#!/usr/bin/env python3
"""
Standalone Data Generator for HCM Full Model
DGP: All 4 latent variables with interactions on fee and duration sensitivity

True Parameters:
    ASC_paid = 5.0
    B_FEE = -0.08
    B_FEE_PatBlind = -0.10, B_FEE_PatConst = -0.04
    B_FEE_SecDL = 0.05, B_FEE_SecFP = 0.04
    B_DUR = -0.08
    B_DUR_PatBlind = 0.06, B_DUR_PatConst = 0.04
    B_DUR_SecDL = -0.05, B_DUR_SecFP = -0.04

Latent Variable Structure:
    pat_blind = 0.20*(age_idx-2) - 0.15*(income_idx-3) + N(0,1)
    pat_constructive = 0.25*(edu_idx-3) + N(0,1)
    sec_dl = 0.18*(edu_idx-3) + 0.12*(income_idx-3) + N(0,1)
    sec_fp = 0.20*(edu_idx-3) + N(0,1)

Measurement Model:
    20 Likert items (5 per construct) with construct-specific loadings
...
def simulate(config_path: Path, output_path: Path, verbose: bool = True) -> pd.DataFrame:
    """Main simulation function for HCM Full."""
    ...
    for i in range(N):
        ...
        # Generate all 4 latent variables and Likert items using shared DGP
...
def main():
    ...
    print("=" * 60)
    print("HCM Full - Data Simulation")
    print("=" * 60)
    print("Generating data with 4 latent variables:")
